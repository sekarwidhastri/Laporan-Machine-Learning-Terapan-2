...
Dataset ini berisi informasi untuk 8.494 produk unik, dengan data inti seperti ID produk, nama, brand, harga, dan kategori utama yang sepenuhnya lengkap, menyediakan fondasi yang kuat untuk analisis. Meskipun demikian, terdapat tantangan data yang hilang pada beberapa fitur penting; kolom seperti rating, ingredients, dan size memiliki tingkat kelengkapan yang cukup baik tetapi tetap memerlukan penanganan data kosong (NaN). Masalah yang lebih signifikan terletak pada kolom-kolom yang sangat jarang terisi (sparse) seperti `sale_price_usd`, `value_price_usd`, dan `variation_desc`, yang mengindikasikan bahwa fitur-fitur ini hanya berlaku untuk sebagian kecil produk dan harus digunakan dengan hati-hati atau diabaikan dalam pemodelan umum untuk menghindari hasil yang tidak akurat.

**Kondisi Dataset Ulasan Pertama**

Variabel-variabel utama yang terdapat pada dataset ulasan pertama adalah sebagai berikut:
...
Dataset ini terdiri dari 206.725 ulasan dan memiliki fondasi yang sangat kuat untuk pemodelan, karena kolom-kolom inti seperti `author_id`, `product_id`, dan `rating` sepenuhnya lengkap tanpa ada data yang hilang. Namun, tantangan utama terletak pada data tambahan yang tidak lengkap; atribut profil pengguna seperti `skin_type` dan `skin_tone` memiliki sekitar 10-20% data yang kosong, sementara metrik `helpfulness` ulasan bahkan hilang lebih dari setengah. Hal ini mengindikasikan bahwa sementara model rekomendasi utama dapat dibangun dengan andal, personalisasi yang lebih mendalam berdasarkan atribut pengguna atau kualitas ulasan akan memerlukan strategi penanganan data kosong yang cermat, dan kolom `Unnamed: 0` yang merupakan sisa indeks sebaiknya dihapus.

**Kondisi Dataset Ulasan Kedua**
...
Dataset ini berisi 116.262 ulasan yang memiliki fondasi data yang sangat solid, di mana kolom-kolom penting untuk sistem rekomendasi seperti `author_id`, `rating`, dan `product_id` sepenuhnya lengkap. Ini menunjukkan data sangat andal untuk pemodelan inti. Namun, tantangannya terletak pada kelengkapan data-data sekunder; atribut profil pengguna seperti `skin_type` dan `skin_tone` memiliki sekitar 8-15% data yang hilang, sementara metrik `helpfulness` bahkan hampir separuhnya kosong. Hal ini mengimplikasikan bahwa meskipun model utama dapat dibangun, fitur-fitur personalisasi tambahan atau yang berbasis kualitas ulasan memerlukan penanganan data kosong secara cermat, serta penghapusan kolom `Unnamed: 0` yang merupakan sisa dari proses impor data.

**Kondisi Dataset Gabungan Bersih**
...
Dataset ini menunjukkan bahwa proses pembersihan data telah berhasil dengan sangat baik, menghasilkan DataFrame `df_clean` yang berisi 313.566 baris data yang siap untuk dimodelkan. Hal yang paling signifikan adalah dataset ini sekarang sepenuhnya bersih tanpa ada nilai yang hilang (missing values) di 9 kolom esensial yang telah dipilih. Setiap kolom memiliki Non-Null Count yang sama dengan jumlah total entri, yang berarti terdapat data `author_id`, `user_rating`, dan `product_id` untuk Collaborative Filtering, serta data `ingredients`, `category`, dan `brand_name` yang lengkap untuk Content-Based Filtering, membuat dataset ini dalam kondisi ideal untuk tahap pengembangan model selanjutnya.
...
Model Content-Based Filtering menghasilkan output sebagai berikut:

![image](https://github.com/user-attachments/assets/583ee6b5-f59f-41a9-986c-786f36764aa1)

Model Collaborative Filtering menghasilkan output sebagai berikut:

![image](https://github.com/user-attachments/assets/fb4202a1-4022-40d8-a041-47deeeed37c8)

Model Popularity-Based Filtering menghasilkan output sebagai berikut:

![image](https://github.com/user-attachments/assets/a9114adf-c448-40cf-8dd1-edd847516ea8)
...
2. **Evaluasi Model Collaborative Filtering (RMSE)**
...
3. **Evaluasi Model Popularity-Based (Baseline)**

Metrik yang digunakan adalah Precision@K, sama seperti pada evaluasi Content-Based. Untuk evaluasi ini, "rekomendasi" adalah daftar Top-5 produk paling populer yang sama yang diberikan kepada setiap pengguna di dalam data uji. Sebuah rekomendasi dianggap "relevan" untuk seorang pengguna jika produk tersebut ada di dalam daftar produk yang benar-benar disukai oleh pengguna tersebut (didefinisikan sebagai produk yang diberi rating 4 atau 5). Hasil evaluasi dari model Popularity-Based ini adalah sebagai berikut:

Rata-rata Precision@5 sebesar 0.45%

Hasil yang sangat rendah ini sudah diharapkan dan justru sangat berguna, karena secara kuantitatif membuktikan bahwa pendekatan "satu untuk semua" dengan merekomendasikan item populer tidak efektif dalam memenuhi selera personal setiap pengguna. Kurang dari 1% rekomendasi populer yang benar-benar cocok dengan preferensi pribadi pengguna. Skor 0.45% ini menjadi tolok ukur performa minimum yang harus bisa dilampaui oleh model Collaborative Filtering yang lebih canggih.
